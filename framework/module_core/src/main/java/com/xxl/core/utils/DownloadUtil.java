package com.xxl.core.utils;

import java.io.File;
import java.io.FileOutputStream;
import java.io.IOException;
import java.io.InputStream;

import okhttp3.Call;
import okhttp3.Callback;
import okhttp3.OkHttpClient;
import okhttp3.Request;
import okhttp3.Response;
import okhttp3.ResponseBody;

/**
 * @author xxl.
 * @date 2022/06/06.
 */
public class DownloadUtil {

    private static DownloadUtil downloadUtil;
    private final OkHttpClient mOkHttpClient;

    public static DownloadUtil get() {
        if (downloadUtil == null) {
            downloadUtil = new DownloadUtil();
        }
        return downloadUtil;
    }

    public DownloadUtil() {
        mOkHttpClient = new OkHttpClient();
    }

    /**
     * @param url          下载连接
     * @param destFileDir  下载的文件储存目录
     * @param destFileName 下载文件名称，后面记得拼接后缀，否则手机没法识别文件类型
     * @param listener     下载监听
     */
    public void download(final String url, final String destFileDir, final String destFileName, final OnDownloadListener listener) {
        final Request request = new Request.Builder()
                .url(url)
                .build();

        //异步请求
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                // 下载失败监听回调
                listener.onDownloadFailed(e);
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                InputStream is = null;
                byte[] buf = new byte[2048];
                int len = 0;
                FileOutputStream fos = null;
                //储存下载文件的目录
                File dir = new File(destFileDir);
                if (!FileUtils.createOrExistsDir(destFileDir)) {
                    listener.onDownloadFailed(new Exception("创建文件夹失败"));
                    return;
                }
                File file = new File(dir, destFileName);
                LogUtils.d("download " + file);
                try {
                    final ResponseBody body = response.body();
                    if (body == null) {
                        listener.onDownloadFailed(new Exception("未找到文件"));
                        return;
                    }
                    is = body.byteStream();
                    long total = body.contentLength();
                    fos = new FileOutputStream(file);
                    long sum = 0;
                    while ((len = is.read(buf)) != -1) {
                        fos.write(buf, 0, len);
                        sum += len;
                        //下载中更新进度条
                        listener.onDownloading(total, sum);
                    }
                    fos.flush();
                    //下载完成
                    listener.onDownloadSuccess(file);
                } catch (Exception e) {
                    listener.onDownloadFailed(e);
                } finally {
                    try {
                        if (is != null) {
                            is.close();
                        }
                        if (fos != null) {
                            fos.close();
                        }
                    } catch (IOException e) {
                        LogUtils.e("下载文件失败" + e.getMessage());
                    }
                }
            }
        });
    }


    public interface OnDownloadListener {

        /**
         * 下载成功之后的文件
         *
         * @param file
         */
        void onDownloadSuccess(File file);

        /**
         * 下载中
         *
         * @param progress
         */
        default void onDownloading(int progress) {

        }

        /**
         * 下载中
         *
         * @param totalSize
         * @param currentSize
         */
        default void onDownloading(long totalSize,
                                   long currentSize) {

            int progress = (int) (currentSize * 1.0f / totalSize * 100);
            onDownloading(progress);
        }

        /**
         * 下载异常信息
         *
         * @param e
         */
        void onDownloadFailed(Throwable e);
    }
}