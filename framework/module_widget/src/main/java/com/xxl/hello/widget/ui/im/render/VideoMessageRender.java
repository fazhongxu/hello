 ...
 * @date 2025/12/5.
 */
public class VideoMessageRender extends BaseMessageRender<WidgetRecycleItemMessageVideoBinding> {
    ...
    private VideoMessageRender() {

    }

    public final static VideoMessageRender obtain() {
        return new VideoMessageRender();
    }
    ...
    public String getRenderTag() {
        return String.valueOf(MessageType.VIDEO);
    }
     ...
     */
    @Override
    public void render(WidgetRecycleItemMessageVideoBinding messageBinding, MessageEntity messageEntity, OnMessageTemplateListener listener) {
        setupImageView(messageBinding, messageEntity);
        messageBinding.ivCover.setOnClickListener(v -> {
            if (listener != null && listener.onMessageItemClick(messageEntity)) {
                return;
            }
        });
        messageBinding.ivCover.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
     ...
     * @param messageEntity
     */
    protected void setupImageView(WidgetRecycleItemMessageVideoBinding videoBinding,
                                  MessageEntity messageEntity) {
        ImageLoader.with(videoBinding.ivCover)
                .asBitmap()
                .load(messageEntity.getMediaPath())
                .into(new CustomTarget<Bitmap>() {
                    @Override
                    public void onResourceReady(@NonNull Bitmap resource, @Nullable Transition<? super Bitmap> transition) {
                        videoBinding.ivCover.setImageBitmap(resource);
                        adjustViewBounds(videoBinding.ivCover, resource.getWidth(), resource.getHeight());
                    }
